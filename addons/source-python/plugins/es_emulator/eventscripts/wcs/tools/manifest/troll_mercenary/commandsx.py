...
def set_model(command):
    ...
    if model == '0':
        for index in _game_models:
            if userid == _game_models[index]:
    ...
    _game_models[entity.index] = player.userid
...
@Event('player_spawn')
def player_spawn(ev):
	wcsgroup.setUser(ev['userid'],'headshot_immunity',0)
...
def player_death(ev):
    ...
    for index in list(_game_models.keys()):
        if _game_models[index] == userid:
            Entity(index).call_input('FireUser1', '1')
 
            del _game_models[index]
